...
public:
    ...
    CBigNum(int64 n)            { m_bn = BN_new(); setint64(n); }
    ...
    CBigNum(uint64 n)           { m_bn = BN_new(); setuint64(n); }
    ...
    unsigned int GetCompact() const
    {
        ...
        {
            CBigNum bn;
            BN_rshift(bn.get(), m_bn, 8*(nSize-3));
    ...
    {
        CAutoBN_CTX pctx;
        if (!BN_mul(m_bn, m_bn, b.get(), pctx))
    ...
    CBigNum& operator--()
    {
        // prefix operator
        CBigNum r;
        if (!BN_sub(r.get(), m_bn, BN_value_one()))
...
inline const CBigNum operator%(const CBigNum& a, const CBigNum& b)
{
    CAutoBN_CTX pctx;
    CBigNum r;
    if (!BN_mod(r.get(), a.get(), b.get(), pctx))
        throw bignum_error("CBigNum::operator% : BN_div failed");
    return r;
}
